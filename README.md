...
#### Authorization
	...
	**NEW_PASSWORD_REQUIRED response**
	 ...
	 `aws cognito-idp admin-initiate-auth  --user-pool-id {COGNITO_USER_POOL_ID} --client-id {APP_CLIENT_ID} --auth-flow ADMIN_NO_SRP_AUTH --auth-parameters USERNAME={USERNAME},PASSWORD={NEW_PASS}`
